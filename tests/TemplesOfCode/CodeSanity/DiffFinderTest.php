 ...
 * @package TemplesOfCode\CodeSanity\Test
 */
class MockFinder5 extends DiffFinder
{
    /**
     * @param string $location
     * @return  Location|null
     */
    public function resolveLocationAccessor($location)
    {
...
class DiffFinderTest extends \PHPUnit_Framework_TestCase
{
     ...
     *
     */
    public function testConstructionWithArgs()
    {
        $targets = new ArrayCollection();
        $targets->add('/dir4/dir5');
        $targets->add('user1@host1:/dir6/dir7');


        $finder = new DiffFinder('/dir1/dir2/dir3', $targets);

        $sot = $finder->getSourceOfTruth();
        $this->assertInstanceOf('\TemplesOfCode\CodeSanity\Location', $sot);

        $targets = $finder->getTargetLocations();
        $target1 = $targets->get(0);
        $this->assertInstanceOf('\TemplesOfCode\CodeSanity\Location\LocalLocation', $target1);
        $target2 = $targets->get(1);
        $this->assertInstanceOf('\TemplesOfCode\CodeSanity\Location\RemoteLocation', $target2);
    }
}
